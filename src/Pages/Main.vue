<template>
  <div class="flex justify-center m-5">
    <input
      class="px-6 py-2 text-[1rem] border-[1px] border-[rgb(210,208,190)] outline-[#f0eaea] rounded-[50px] [box-shadow:0_3px_3px_rgb(237,_240,_239)] w-[450px]"
      type="text" v-model="searchTerm" @keyup="searchProduct" placeholder="Search for Product..." />
  </div>
  <div class="flex flex-wrap justify-center items-center w-full">
    <div v-for="product in productList" :key="product.id"
      ...
      <router-link :to="{ path: '/product/' + product.id, query: { product_id: product.id } }">
        <img :src="product.images[0]" :alt="product.title" class="h-[345px] w-[282px] m-[10px] flex" />
        <div class="h-28 w-[100%]">
          <h1 class="text-lg text-[#e84409] font-medium">{{ product.title }}</h1>
